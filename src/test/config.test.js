...
describe('site.config — brand identity contract', () => {
  ...
  it('contact exposes email, phone and a location for LocalBusiness schema', () => {
    expect(site.contact.email).toBeTruthy()
    expect(site.contact.phone).toBeTruthy()
    expect(site.contact.location).toBeTruthy()
  })

  it('social exposes at least one network', () => {
    const networks = Object.values(site.social).filter(Boolean)
